...
7. Execute the contents of the files in the `sql` folder:
   * The contents of the first script (`01-create-logins.sql`) should be executed against the `master` database
   * The contents of the rest of the scripts should be executed against the `SQLPool01` database
...
5. Modify the query as shown below to skip the header row and specify the names of the columns in the resultset; replacing *SUFFIX* with the unique resource suffix for your storage account:
    ...
    FROM
        OPENROWSET(
            BULK 'https://YOUR_STORAGE_ACCOUNT_NAME.dfs.core.windows.net/wwi-02/sale-poc/sale-20170501.csv',
            FORMAT = 'CSV',
            PARSER_VERSION='2.0',
            FIRSTROW = 2
        ) WITH (
...
3. Import the **Explore with Spark.ipynb** notebook in the `synapse-apache-spark-notebooks` folder.
...
4. Add the following beneath the code in the cell to define a variable named **datalake** whose value is the name of the primary storage account (replace *SUFFIX* with the unique suffix for your data store):
    ...
    ```python
    datalake = 'YOUR_STORAGE_ACCOUNT_NAME'
    ```
